...
impl Server {
    pub fn new(config: Config) -> Result<Server, ProxyError> {
        Ok(Server {
    ...
    async fn run_process(
        ...
        let listener = match TcpListener::bind(&self.listen_addr).await {
            ...
            Err(err) => {
                warn!("Failed to bind listener on {}: {err}", self.listen_addr);
    ...
    async fn socks_read_request(
        ...
        if cmd != SocksCommand::CONNECT {
            Self::socks_write_error_response(socket, CommandResponse::COMMAND_NOT_SUPPORTED)
                .await?;
            debug!("Command {cmd} is not supported");
...
impl fmt::Display for ProxyError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::Join(err) => write!(f, "Tokio join error: {err}"),
...
impl From<network::NetworkError> for ProxyError {
    fn from(err: network::NetworkError) -> ProxyError {
        Self::Network(err)
    }
}
